...
class StebarnsadopsjonDelRegelTest {
    ...
    void UT1240_stebarnsadopsjon_far_ikke_omsorg() {
        ...
        var uttakPeriode =
                oppgittPeriode(omsorgsovertakelseDato, omsorgsovertakelseDato.plusWeeks(2));

        var grunnlag =
                grunnlagFar(omsorgsovertakelseDato, uttakPeriode)
                        .rettOgOmsorg(
    ...
    void UT1241_stebarnsadopsjon_far_omsorg_disponible_dager_og_ingen_gradering() {
        ...
        assertThat(regelresultat.getManuellbehandlingårsak())
                .isEqualTo(Manuellbehandlingårsak.STEBARNSADOPSJON);
    }
    ...
    void UT1285_stebarnsadopsjon_uttak_før_omsorgsovertakelse() {
        ...
        var uttakPeriode =
                oppgittPeriode(
                        omsorgsovertakelseDato.minusDays(3), omsorgsovertakelseDato.plusWeeks(2));
        ...
        assertThat(regelresultat.skalUtbetale()).isFalse();
        assertThat(regelresultat.getAvklaringÅrsak())
                .isEqualTo(IkkeOppfyltÅrsak.FØR_OMSORGSOVERTAKELSE);
    }

    private RegelGrunnlag.Builder grunnlagFar(
            LocalDate familiehendelseDato, OppgittPeriode oppgittPeriode) {
        var kontoer =
                new Kontoer.Builder()
                        .konto(new Konto.Builder().type(MØDREKVOTE).trekkdager(50))
