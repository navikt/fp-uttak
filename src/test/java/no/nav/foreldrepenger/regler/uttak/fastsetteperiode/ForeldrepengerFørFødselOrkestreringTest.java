...
class ForeldrepengerFørFødselOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    void foreldrepengerFørFødsel_happy_case() {
        ...
        var fpff = oppgittPeriode(
                Stønadskontotype.FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1));
    ...
    void foreldrepengerFørFødsel_far_søker_fpff() {
        ...
        assertThat(perioder.get(0).uttakPeriode().getStønadskontotype())
                .isEqualTo(Stønadskontotype.FORELDREPENGER_FØR_FØDSEL);
        assertThat(perioder.get(0).uttakPeriode().getManuellbehandlingårsak())
    ...
    void foreldrepengerFørFødsel_for_lang_fpff_periode_før_fødsel() {
        ...
        assertThat(perioder.get(0).uttakPeriode().getTom())
                .isEqualTo(fødselsdato.minusWeeks(3).minusDays(1));
    ...
    void foreldrepengerFørFødsel_for_lang_fpff_periode_etter_fødsel() {
        ...
        assertThat(perioder.get(1).uttakPeriode().getFom()).isEqualTo(fødselsdato);
        assertThat(perioder.get(1).uttakPeriode().getTom())
                .isEqualTo(fødselsdato.plusWeeks(2).minusDays(1));
        assertThat(perioder.get(1).innsendtGrunnlag()).isNotNull();
        assertThat(perioder.get(1).evalueringResultat()).isNotNull();

        // Mødrekvoten blir satt til manuell pga forrige periode ble manuell. Ingen årsak eller
        // regelvurdering.
    ...
    void foreldrepengerFørFødsel_for_kort_fpff_periode_slutter_for_tidlig() {
        ...
        assertThat(perioder.get(0).uttakPeriode().getManuellbehandlingårsak())
                .isEqualTo(Manuellbehandlingårsak.UGYLDIG_STØNADSKONTO);
        assertThat(perioder.get(0).uttakPeriode().getFom())
                .isEqualTo(fødselsdato.minusWeeks(3).minusDays(1));
        assertThat(perioder.get(0).uttakPeriode().getTom())
                .isEqualTo(fødselsdato.minusWeeks(3).minusDays(1));
        assertThat(perioder.get(0).innsendtGrunnlag()).isNotNull();
