...
class UtsettelseSammenhengendeUttakOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    void periode_med_gyldig_utsettelse_pga_barn_innlagt_i_helseinstitusjon_skal_innvilges() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                ...
                .oppgittPeriode(
                    utsettelsePeriode(fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1), INNLAGT_BARN, INNLEGGELSE_BARN_GODKJENT)));
    ...
    void periode_med_gyldig_utsettelse_pga_søker_innlagt_i_helseinstitusjon_skal_innvilges() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                .oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(10).minusDays(1)))
                .oppgittPeriode(
                    utsettelsePeriode(fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1), INNLAGT_SØKER, INNLEGGELSE_SØKER_GODKJENT)));
    ...
    void periode_med_gyldig_utsettelse_pga_søkers_sykdom_eller_skade_skal_innvilges() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                .oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(10).minusDays(1)))
                .oppgittPeriode(
                    utsettelsePeriode(fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1), SYKDOM_SKADE, SYKDOM_SØKER_GODKJENT)));
    ...
    void periode_med_gyldig_utsettelse_pga_arbeid_skal_innvilges() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).arbeid(
                new Arbeid.Builder().arbeidsforhold(arbeidsforhold).endringAvStilling(new EndringAvStilling(fødselsdato, BigDecimal.valueOf(100))))
            .kontoer(kontoer)
            .søknad(fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
    ...
    void UT1110_periode_med_utsettelse_pga_arbeid_med_50_prosent_stilling_skal_manuell_behandles() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).kontoer(kontoer)
            .søknad(fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                ...
                .oppgittPeriode(utsettelsePeriode(utsettelseFom, utsettelseTom, ARBEID, null)))
            .arbeid(
                new Arbeid.Builder().arbeidsforhold(arbeidsforhold).endringAvStilling(new EndringAvStilling(utsettelseFom, BigDecimal.valueOf(50))));
    ...
    void periode_med_utsettelse_pga_ferie_skal_innvilges() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                .oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(10).minusDays(1)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1), FERIE, null)));
    ...
    void flere_perioder_med_utsettelse_pga_ferie_skal_til_manuell_behandling_grunnet_bevegelige_helligdager() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                .oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(10).minusDays(1)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1), FERIE, null))
                .oppgittPeriode(oppgittPeriode(FELLESPERIODE, fødselsdato.plusWeeks(12), fødselsdato.plusWeeks(17).minusDays(1)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(17), fødselsdato.plusWeeks(18).minusDays(1), FERIE, null)));
    ...
    void periode_med_ugyldig_utsettelse_skal_til_manuell_behandling() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                .oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(10).minusDays(1)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1), INNLAGT_BARN, null)));
    ...
    void utsettelse_uten_stønadskonto_på_helligdag_skal_gi_ugyldig_opphold() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                ...
                .oppgittPeriode(utsettelsePeriode(LocalDate.of(2018, 12, 25), LocalDate.of(2018, 12, 25), FERIE, null)));
    ...
    void utsettelse_periode_med_ukjent_kontotype_må_settes_til_neste_tilgjengelig() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER_FØR_FØDSEL, fødselsdato.minusWeeks(3), fødselsdato.minusDays(1)))
                .oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(10).minusDays(1)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1), SYKDOM_SKADE, null)));
    ...
    void pleiepenger_utsettelse_skal_trekke_fra_fellesperiode() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).datoer(new Datoer.Builder().termin(termindato).fødsel(fødselsdato)).søknad(fødselSøknad()
    ...
    void skal_trekke_fra_mødrekvote_ved_avslag_ferie_innenfor_første_seks_uker() {
        ...
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).datoer(new Datoer.Builder().fødsel(fødselsdato))
            .søknad(fødselSøknad().oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(4)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(4).plusDays(1), fødselsdato.plusWeeks(8), FERIE, null)));
    ...
    void skal_trekke_fra_foreldrepengekvote_ved_avslag_ferie_innenfor_første_seks_uker_ved_aleneomsorg_far() {
        var fødselsdato = LocalDate.of(2019, 7, 1);
        var grunnlag = aleneomsorgUtsettelseGrunnlag(fødselsdato, farBehandling()).datoer(new Datoer.Builder().fødsel(fødselsdato))
    ...
    void skal_trekke_fra_foreldrepengekvote_ved_avslag_ferie_innenfor_første_seks_uker_ved_aleneomsorg_mor() {
        var fødselsdato = LocalDate.of(2019, 7, 1);
        var grunnlag = aleneomsorgUtsettelseGrunnlag(fødselsdato, morBehandling()).datoer(new Datoer.Builder().fødsel(fødselsdato))
            .søknad(fødselSøknad().oppgittPeriode(oppgittPeriode(FORELDREPENGER, fødselsdato, fødselsdato.plusWeeks(4)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(4).plusDays(1), fødselsdato.plusWeeks(8), FERIE, null)));
    ...
    void utsettelse_før_søknad_mottatt_dato_skal_innvilges() {
        var fødselsdato = LocalDate.of(2019, 7, 1);
        var utsettelse = OppgittPeriode.forUtsettelse(fødselsdato.plusWeeks(6), fødselsdato.plusWeeks(10), FERIE, fødselsdato.plusWeeks(8),
            fødselsdato.plusWeeks(8), null, null);
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(6).minusDays(1))).oppgittPeriode(utsettelse));
    ...
    void skal_bruke_tidligst_mottatt_dato_når_søknadsfrist_vurderes() {
        ...
        var utsettelse = OppgittPeriode.forUtsettelse(fødselsdato.plusWeeks(6), fødselsdato.plusWeeks(10), FERIE, senestMottattDato,
            tidligstMottattDato, null, null);
        var grunnlag = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(6).minusDays(1))).oppgittPeriode(utsettelse));
    ...
    void utsettelse_ved_tomme_dager_skal_også_avslås() {
        ...
        var uttakAvsluttetMedUtsettelse = basicUtsettelseGrunnlag(fødselsdato).søknad(
            fødselSøknad().oppgittPeriode(oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(10).minusDays(1)))
                .oppgittPeriode(oppgittPeriode(FELLESPERIODE, fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(36).minusDays(1)))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(36), fødselsdato.plusWeeks(37).minusDays(1), FERIE, null))
                .oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(37), fødselsdato.plusWeeks(100), ARBEID, null))).build();
    ...
    void skal_knekke_riktig_på_helligdager_i_jula_ved_utsettelse_pga_ferie() {
        ...
        var grunnlag = basicGrunnlagFar(fødselsdato).annenPart(new AnnenPart.Builder().uttaksperiode(
                AnnenpartUttakPeriode.Builder.uttak(fødselsdato, fødselsdato.plusWeeks(6).minusDays(1))
                    ...
                    .build()))
            .søknad(
                new Søknad.Builder().type(FØDSEL).oppgittPeriode(utsettelsePeriode(fødselsdato.plusWeeks(6), LocalDate.of(2020, 1, 1), FERIE, null)));
        ...
        var grunnlag = basicGrunnlagFar(fødselsdato).annenPart(new AnnenPart.Builder().uttaksperiode(
                AnnenpartUttakPeriode.Builder.uttak(fødselsdato, tiltakFom.minusDays(1))
                    ...
                    .uttakPeriodeAktivitet(
                        new AnnenpartUttakPeriodeAktivitet(AktivitetIdentifikator.forFrilans(), MØDREKVOTE, Trekkdager.ZERO, Utbetalingsgrad.ZERO))
                    .build()))
            .søknad(new Søknad.Builder().type(FØDSEL).oppgittPeriode(utsettelsePeriode(tiltakFom, tiltakTom, NAV_TILTAK, NAV_TILTAK_GODKJENT)));
    ...
    void bfhr_utsettelse_som_trenger_dok_og_aktivitetskrav_vurdering_skal_gå_til_manuell() {
        ...
        var innleggelse = OppgittPeriode.forUtsettelse(fødselsdato.plusWeeks(11), fødselsdato.plusWeeks(12).minusDays(1), INNLAGT_SØKER, fødselsdato,
            fødselsdato, MorsAktivitet.UTDANNING, INNLEGGELSE_SØKER_GODKJENT);

        var sykdomBarn = OppgittPeriode.forUtsettelse(fødselsdato.plusWeeks(12), fødselsdato.plusWeeks(13).minusDays(1), INNLAGT_BARN, fødselsdato,
