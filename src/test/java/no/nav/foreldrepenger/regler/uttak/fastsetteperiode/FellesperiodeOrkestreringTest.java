...
class FellesperiodeOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    @Test
    void
            fellesperiode_mor_etter_uke_7_etter_fødsel_uten_nok_dager_blir_innvilget_med_knekk_og_avslått_periode_på_resten() {
        var kontoer = new Kontoer.Builder()
                .konto(new Konto.Builder().type(FORELDREPENGER_FØR_FØDSEL).trekkdager(1000))
    ...
    private Kontoer.Builder fellesperiodeKonto(int trekkdager) {
        return new Kontoer.Builder()
                .konto(new Konto.Builder().type(FELLESPERIODE).trekkdager(trekkdager));
    }
    ...
    void fellesperiode_far_etter_uke_6_blir_innvilget_pga_oppfyller_aktivitetskravet() {
        ...
        var oppgittPeriode = oppgittPeriode(
                FELLESPERIODE, fødselsdato.plusWeeks(6), fødselsdato.plusWeeks(15), MORS_AKTIVITET_GODKJENT);
    ...
    void fellesperiode_mor_uttak_starter_ved_12_uker_og_slutter_etter_3_uker_før_fødsel_og_blir_innvilget() {
        var kontoer = new Kontoer.Builder()
                .konto(new Konto.Builder().type(FORELDREPENGER_FØR_FØDSEL).trekkdager(3 * 5))
    ...
    void fellesperiode_mor_uttak_starter_ved_3_uker_etter_fødsel_blir_knekt_ved_6_uker_og_må_behandles_manuelt() {
        var kontoer = new Kontoer.Builder()
                .konto(new Konto.Builder().trekkdager(15).type(Stønadskontotype.FORELDREPENGER_FØR_FØDSEL))
        ...
        verifiserAvslåttPeriode(
                resultater.get(0).uttakPeriode(),
                fødselsdato,
                fødselsdato.plusWeeks(3).minusDays(3),
                MØDREKVOTE,
    ...
    void fellesperiode_mor_uttak_starter_før_12_uker_blir_avslått_med_knekk_ved_12_uker_før_fødsel() {
        var kontoer = new Kontoer.Builder()
                .konto(new Konto.Builder().type(Stønadskontotype.MØDREKVOTE).trekkdager(1000))
                .konto(new Konto.Builder().type(Stønadskontotype.FELLESPERIODE).trekkdager(13 * 5));
        var grunnlag = basicGrunnlagMor()
                .søknad(søknad(
                        Søknadstype.FØDSEL,
                        oppgittPeriode(Stønadskontotype.FELLESPERIODE, fødselsdato.minusWeeks(13), fødselsdato)))
                .kontoer(kontoer);
        ...
        verifiserPeriode(
                resultater.get(0).uttakPeriode(),
                fødselsdato.minusWeeks(13),
                fødselsdato.minusWeeks(12).minusDays(1),
        ...
        verifiserManuellBehandlingPeriode(
                ...
                Manuellbehandlingårsak.UGYLDIG_STØNADSKONTO);
        verifiserAvslåttPeriode(
                resultater.get(4).uttakPeriode(),
                fødselsdato.plusDays(1),
                fødselsdato.plusWeeks(6).minusDays(3),
                MØDREKVOTE,
                IkkeOppfyltÅrsak.MOR_TAR_IKKE_UKENE_ETTER_FØDSEL);
    }
