...
class SporingOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    void fastsette_perioder_regel_skal_produsere_sporing_i_json_format()
        ...
        for (var resultat : resultatListe) {
            assertThat(new ObjectMapper().readValue(resultat.innsendtGrunnlag(), HashMap.class))
                    .isNotNull()
                    .isNotEmpty();
            assertThat(new ObjectMapper().readValue(resultat.evalueringResultat(), HashMap.class))
                    .isNotNull()
                    .isNotEmpty();
        }
    }
