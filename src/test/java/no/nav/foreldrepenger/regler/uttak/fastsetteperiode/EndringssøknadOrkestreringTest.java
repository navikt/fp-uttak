...
class EndringssøknadOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    void skal_trekke_dager_for_perioder_i_vedtak() {
        ...
        var vedtaksperiode = new FastsattUttakPeriode.Builder()
                .tidsperiode(fødselsdato, endringsdato.minusDays(1))
                .periodeResultatType(Perioderesultattype.INNVILGET)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(30), Stønadskontotype.MØDREKVOTE, aktivitet)));
        ...
        var søknadOm10UkerMødrekvote = søknad(
                Søknadstype.FØDSEL,
                oppgittPeriode(
                        Stønadskontotype.MØDREKVOTE,
                        endringsdato,
                        endringsdato.plusWeeks(10).minusDays(1)));
        ...
        assertThat(resultat.get(1).uttakPeriode().getPerioderesultattype()).isNotEqualTo(Perioderesultattype.INNVILGET);
    ...
    void skal_arve_høyest_saldo_fra_for_arbeidsforhold_med_startdato_i_uttak() {
        ...
        var opprinnligUttakAktivitet1 =
                new FastsattUttakPeriodeAktivitet(new Trekkdager(30), Stønadskontotype.MØDREKVOTE, aktivitet1);
        var opprinnligUttakAktivitet2 =
                new FastsattUttakPeriodeAktivitet(new Trekkdager(20), Stønadskontotype.MØDREKVOTE, aktivitet2);
        var vedtaksperiode = new FastsattUttakPeriode.Builder()
                .tidsperiode(fødselsdato, fødselsdato.plusWeeks(6).minusDays(1))
                .periodeResultatType(Perioderesultattype.INNVILGET)
                .aktiviteter(List.of(opprinnligUttakAktivitet1, opprinnligUttakAktivitet2));
        var vedtak = new Vedtak.Builder().leggTilPeriode(vedtaksperiode);
        var revurdering =
                new Revurdering.Builder().endringsdato(fødselsdato.plusWeeks(6)).gjeldendeVedtak(vedtak);
        var søknadOm12UkerMødrekvote = søknad(
                Søknadstype.FØDSEL,
                oppgittPeriode(
                        Stønadskontotype.MØDREKVOTE,
        ...
        var grunnlag = basicGrunnlagMor(fødselsdato)
                ...
                .arbeid(new Arbeid.Builder()
                        .arbeidsforhold(new Arbeidsforhold(aktivitet1))
