...
class AvslagAktivitetskravOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    @ParameterizedTest(
            name =
                    "Ved mors aktivitet {0} og dokumentasjonvurdering er {1}, forventes resultat {2}")
    @MethodSource("dokumentasjonOgAvslagKombinasjoner")
    void
            test_kombinasjoner_av_MorsAktivitet_PeriodeMedAvklartMorsAktivitetResultat_og_IkkeOppfyltÅrsak_sammenheng(
    ...
    @MethodSource("dokumentasjonOgAvslagKombinasjoner")
    void
            test_kombinasjoner_av_MorsAktivitet_PeriodeMedAvklartMorsAktivitetResultat_og_IkkeOppfyltÅrsak_fri(
                    MorsAktivitet morsAktivitet,
                    IkkeOppfyltÅrsak ikkeOppfyltÅrsak,
                    DokumentasjonVurdering dokumentasjonVurdering) {
    ...
    void mor_med_bekreftet_uføretrygd_skal_avslås() {
        ...
        var kontoer =
                new Kontoer.Builder()
                        .konto(new Konto.Builder().type(FORELDREPENGER).trekkdager(200))
                        .minsterettDager(20);
        ...
        assertThat(fastsattePerioder.get(0).uttakPeriode().getPeriodeResultatÅrsak())
                .isEqualTo(InnvilgetÅrsak.FORELDREPENGER_KUN_FAR_HAR_RETT_UTEN_AKTIVITETSKRAV);
    ...
    void mor_med_bekreftet_ikke_uføretrygd_skal_avslås() {
        ...
        var fastsattePerioder = fastsettPerioder(grunnlag);
        assertThat(fastsattePerioder.get(0).uttakPeriode().getPeriodeResultatÅrsak())
                .isEqualTo(FORELDREPENGER_KUN_FAR_HAR_RETT_MOR_IKKE_UFØR);
    ...
    void mor_uten_bekreftet_uføretrygd_skal_til_manuell() {
        ...
        var grunnlag =
                new RegelGrunnlag.Builder()
                        ...
                        .kontoer(kontoer);
        var fastsattePerioder = fastsettPerioder(grunnlag);
        assertThat(fastsattePerioder.get(0).uttakPeriode().getManuellbehandlingårsak())
                .isEqualTo(Manuellbehandlingårsak.MOR_UFØR);
        assertThat(fastsattePerioder.get(0).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD).merEnn0())
                .isFalse();
    }
    ...
    @Test
    void
            ukjent_mors_aktivitet_fri_utsettelse_foreldrepenger_skal_avslås_hvis_minsterett_ikke_dokumentert() {
        var fødselsdato = LocalDate.of(2022, 9, 1);
        ...
        var søknad =
                new Søknad.Builder()
                        ...
                        .oppgittePerioder(List.of(oppgittPeriode, utsettelse1, utsettelse2));
        ...
        assertThat(fastsattePerioder.get(1).uttakPeriode().getPeriodeResultatÅrsak())
                .isEqualTo(AKTIVITET_UKJENT_UDOKUMENTERT);
        assertThat(fastsattePerioder.get(2).uttakPeriode().getManuellbehandlingårsak())
                .isEqualTo(Manuellbehandlingårsak.AKTIVITEKTSKRAVET_MÅ_SJEKKES_MANUELT);
    ...
    private OppgittPeriode fellesperiode(
    ...
    private static Stream<Arguments> dokumentasjonOgAvslagKombinasjoner() {
        return Stream.of(
                Arguments.of(
                        ARBEID, AKTIVITETSKRAVET_ARBEID_IKKE_OPPFYLT, MORS_AKTIVITET_IKKE_GODKJENT),
                Arguments.of(
                        ARBEID,
                        AKTIVITETSKRAVET_ARBEID_IKKE_DOKUMENTERT,
                        MORS_AKTIVITET_IKKE_DOKUMENTERT),
                Arguments.of(
                        SYK, AKTIVITETSKRAVET_SYKDOM_IKKE_OPPFYLT, MORS_AKTIVITET_IKKE_GODKJENT),
                Arguments.of(
                        SYK,
                        AKTIVITETSKRAVET_SYKDOM_IKKE_DOKUMENTERT,
                        MORS_AKTIVITET_IKKE_DOKUMENTERT),
                Arguments.of(
                        INNLAGT,
                ...
                Arguments.of(
                        UTDANNING,
                        AKTIVITETSKRAVET_UTDANNING_IKKE_OPPFYLT,
                        MORS_AKTIVITET_IKKE_GODKJENT),
                Arguments.of(
                        UTDANNING,
    ...
    private void testAvslag(
        ...
        var grunnlag =
                new RegelGrunnlag.Builder()
                        .behandling(farBehandling().kreverSammenhengendeUttak(sammenhengendeUttak))
                        .datoer(new Datoer.Builder().fødsel(fødselsdato))
                        .rettOgOmsorg(beggeRett())
                        .søknad(søknad)
                        .inngangsvilkår(oppfyltAlleVilkår())
