...
class ManglendeSøktePerioderTjenesteTest {
    ...
    void skalLageManglendeSøktIMellomliggendePerioderNårBareFarHarRett_fødsel() {
        var familiehendelse = LocalDate.of(2021, 9, 27);

        var grunnlag = grunnlagMedKontoer()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL)
                        .oppgittPeriode(oppgittPeriode(FORELDREPENGER, familiehendelse.minusWeeks(3), familiehendelse.plusWeeks(2)))
                        .oppgittPeriode(oppgittPeriode(FORELDREPENGER, familiehendelse.plusWeeks(4), familiehendelse.plusWeeks(8)))
                        .oppgittPeriode(oppgittPeriode(FORELDREPENGER, familiehendelse.plusWeeks(10), familiehendelse.plusWeeks(15)))
                )
                .behandling(farBehandling())
                .rettOgOmsorg(new RettOgOmsorg.Builder().farHarRett(true).morHarRett(false))
                .datoer(new Datoer.Builder().fødsel(familiehendelse))
        ...
        assertThat(msp.get(1).getFom()).isEqualTo(familiehendelse.plusWeeks(8).plusDays(1));
    }
