...
class ArbeidsforholdPeriodeRegelOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    void skal_starte_arbeidsforhold_fra_startdato() {
        ...
        var arbeid = new Arbeid.Builder()
                .arbeidsforhold(new Arbeidsforhold(arbeidsforhold1, omsorgsovertakelse.minusYears(1)))
        ...
        assertThat(aktiviteterIPeriode(resultat.get(3).uttakPeriode()))
                .containsExactlyInAnyOrder(arbeidsforhold1, arbeidsforhold2, arbeidsforhold3, arbeidsforhold4);
    }
    ...
    void nytt_arbeidsforhold_skal_hente_saldo_fra_eksisterende_arbeidsforhold() {
        ...
        var gradertMødrekvote = gradertoppgittPeriode(
                MØDREKVOTE,
                fødselsdato.plusWeeks(6),
                fødselsdato.plusWeeks(8).minusDays(1),
                BigDecimal.valueOf(50),
                Set.of(arbeidsforhold1));
        var mødrekvote2 = oppgittPeriode(
                MØDREKVOTE, fødselsdato.plusWeeks(8), fødselsdato.plusWeeks(11).minusDays(1));
        var grunnlag = basicGrunnlag()
                .arbeid(arbeid)
    ...
    void nytt_arbeidsforhold_skal_hente_saldo_fra_arbeidsforholdet_som_har_høyest_saldo_på_startdato_2() {
        ...
        var mødrekvote1 =
                oppgittPeriode(MØDREKVOTE, fødselsdato, fødselsdato.plusWeeks(6).minusDays(1));
        var gradertMødrekvote1 = gradertoppgittPeriode(
                MØDREKVOTE,
                fødselsdato.plusWeeks(6),
                fødselsdato.plusWeeks(8).minusDays(1),
                BigDecimal.valueOf(50),
        ...
        var mødrekvote2 = oppgittPeriode(
                MØDREKVOTE, fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(12).minusDays(1));
        var grunnlag = basicGrunnlag()
                .arbeid(arbeid)
    ...
    void skal_kunne_lage_manglende_søkt_periode_i_periode_uten_at_søker_har_aktive_arbeidsforhold() {
        ...
        var arbeid = new Arbeid.Builder()
                .arbeidsforhold(new Arbeidsforhold(tilkommetArbeidsforhold1, LocalDate.of(2019, 5, 1)))
                .arbeidsforhold(new Arbeidsforhold(tilkommetArbeidsforhold2, LocalDate.of(2019, 6, 10)));
        var utsettelseArbeid = utsettelsePeriode(
                LocalDate.of(2019, 11, 4),
                LocalDate.of(2019, 12, 6),
                UtsettelseÅrsak.ARBEID,
                null,
                MORS_AKTIVITET_GODKJENT);
        var fpPeriode = oppgittPeriode(
                FORELDREPENGER, LocalDate.of(2019, 12, 16), LocalDate.of(2020, 1, 3), MORS_AKTIVITET_GODKJENT);
        ...
        // manglende søkt
        assertThat(resultat.get(0).uttakPeriode().getAktiviteter().stream()
                        .findFirst()
                        .orElseThrow()
                        .getIdentifikator())
