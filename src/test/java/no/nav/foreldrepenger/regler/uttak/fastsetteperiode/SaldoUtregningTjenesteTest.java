...
class SaldoUtregningTjenesteTest {
    ...
    void skal_knekke_annenparts_perioder() {
        ...
        var annenpartUttaksperiode = AnnenpartUttakPeriode.Builder.uttak(fomAnnenpart, tomAnnenpart)
                ...
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(50), Utbetalingsgrad.FULL))
                .build();
        ...
        var grunnlag = new RegelGrunnlag.Builder()
                .annenPart(new AnnenPart.Builder().uttaksperiode(annenpartUttaksperiode))
                .kontoer(kontoer)
    ...
    void skal_ikke_ta_med_opphold_på_annenpart_hvis_overlapp_med_søkers_periode_og_berørt_behandling() {
        ...
        var annenpartOpphold = AnnenpartUttakPeriode.Builder.opphold(
                        fomAnnenpartOpphold, tomAnnenpartOpphold, FEDREKVOTE_ANNEN_FORELDER)
                .innvilget(true)
                .build();
        var annenpartUttaksperiode = AnnenpartUttakPeriode.Builder.uttak(
                        fomAnnenpartOpphold.minusWeeks(1), fomAnnenpartOpphold.minusWeeks(1))
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(50), Utbetalingsgrad.HUNDRED))
                .build();
    ...
    @Test
    void
            skal_ta_med_deler_av_opphold_på_annenpart_hvis_overlapp_med_søkers_periode_og_berørt_behandling_tidlig_overlapp() {
        var fomAnnenpartOpphold = LocalDate.of(2019, 12, 2);
        var tomAnnenpartOpphold = fomAnnenpartOpphold.plusWeeks(10).minusDays(1);
        var annenpartOpphold = AnnenpartUttakPeriode.Builder.opphold(
                        fomAnnenpartOpphold, tomAnnenpartOpphold, FEDREKVOTE_ANNEN_FORELDER)
    ...
    @Test
    void
            skal_ta_med_deler_av_opphold_på_annenpart_hvis_overlapp_med_søkers_periode_og_berørt_behandling_sen_overlapp() {
        var fomAnnenpartOpphold = LocalDate.of(2019, 12, 2);
        var tomAnnenpartOpphold = fomAnnenpartOpphold.plusWeeks(10).minusDays(1);
        var annenpartOpphold = AnnenpartUttakPeriode.Builder.opphold(
                        fomAnnenpartOpphold, tomAnnenpartOpphold, FEDREKVOTE_ANNEN_FORELDER)
    ...
    @Test
    void
            skal_ta_med_deler_av_opphold_på_annenpart_hvis_overlapp_med_søkers_periode_og_berørt_behandling_midt_overlapp() {
        var fomAnnenpartOpphold = LocalDate.of(2019, 12, 2);
        var tomAnnenpartOpphold = fomAnnenpartOpphold.plusWeeks(10).minusDays(1);
        var annenpartOpphold = AnnenpartUttakPeriode.Builder.opphold(
                        fomAnnenpartOpphold, tomAnnenpartOpphold, FEDREKVOTE_ANNEN_FORELDER)
        ...
        var kontoer = new Kontoer.Builder().konto(konto(FEDREKVOTE, 100));
        var aktuellPeriode =
                oppgittPeriode(FEDREKVOTE, fomAnnenpartOpphold.plusWeeks(2), tomAnnenpartOpphold.minusWeeks(3));
        var grunnlag = new RegelGrunnlag.Builder()
                .annenPart(
                        new AnnenPart.Builder().uttaksperiode(annenpartOpphold).uttaksperiode(annenpartUttaksperiode))
                .kontoer(kontoer)
    ...
    private SaldoUtregningGrunnlag lagGrunnlag(OppgittPeriode aktuellPeriode, RegelGrunnlag grunnlag) {
        if (grunnlag.getBehandling().isBerørtBehandling()) {
            return SaldoUtregningGrunnlag.forUtregningAvDelerAvUttakBerørtBehandling(
                    List.of(),
                    grunnlag.getAnnenPart().getUttaksperioder(),
                    grunnlag,
                    aktuellPeriode.getFom(),
                    new ArrayList<>(grunnlag.getSøknad().getOppgittePerioder()));
        }
        return SaldoUtregningGrunnlag.forUtregningAvDelerAvUttak(
                List.of(), grunnlag.getAnnenPart().getUttaksperioder(), grunnlag, aktuellPeriode.getFom());
    }
    ...
    void skal_ikke_knekke_annenparts_perioder_ved_berørt_behandling() {
        ...
        var annenpartUttaksperiode = AnnenpartUttakPeriode.Builder.uttak(fomAnnenpart, tomAnnenpart)
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(70), Utbetalingsgrad.HUNDRED))
                .build();
        var kontoer = new Kontoer.Builder().konto(konto(FELLESPERIODE, 100));
        var aktuellPeriode =
                oppgittPeriode(FELLESPERIODE, fomAnnenpart.plusWeeks(5).minusDays(1), tomAnnenpart);
        var grunnlag = new RegelGrunnlag.Builder()
                .annenPart(new AnnenPart.Builder().uttaksperiode(annenpartUttaksperiode))
                .kontoer(kontoer)
                .arbeid(new Arbeid.Builder().arbeidsforhold(new Arbeidsforhold(annenAktivitet())))
    ...
    void skal_ikke_avrunde_før_saldo_arves_til_nytt_arbeidsforhold() {
        ...
        var fastsattPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(utregningsdato.minusWeeks(1), utregningsdato.minusDays(1))
                .periodeResultatType(INNVILGET)
                .aktiviteter(
                        List.of(new FastsattUttakPeriodeAktivitet(new Trekkdager(2.5), FELLESPERIODE, identifikator)))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder()
                        .arbeidsforhold(new Arbeidsforhold(identifikator))
        ...
        var saldoUtregningGrunnlag = SaldoUtregningGrunnlag.forUtregningAvDelerAvUttak(
                List.of(fastsattPeriode), List.of(), grunnlag, utregningsdato);
        var resultat = SaldoUtregningTjeneste.lagUtregning(saldoUtregningGrunnlag);

        assertThat(resultat.saldoITrekkdager(FELLESPERIODE, identifikator)).isEqualTo(new Trekkdager(97.5));
        assertThat(resultat.saldoITrekkdager(FELLESPERIODE, identifikatorNyttArbeidsforhold))
                .isEqualTo(new Trekkdager(97.5));
    }
    ...
    void skal_ikke_dobbelt_trekke_fra_annenpart_når_arbeidsforhold_starter_ila_uttaket() {
        ...
        var fastsattPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(LocalDate.of(2019, 12, 18), LocalDate.of(2019, 12, 19))
                .periodeResultatType(INNVILGET)
                .aktiviteter(
                        List.of(new FastsattUttakPeriodeAktivitet(new Trekkdager(50), FELLESPERIODE, identifikator)))
                .build();
        var annenpartsPeriode = AnnenpartUttakPeriode.Builder.uttak(
                        LocalDate.of(2019, 12, 11), LocalDate.of(2019, 12, 17))
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forSelvstendigNæringsdrivende(), MØDREKVOTE, new Trekkdager(100), Utbetalingsgrad.HUNDRED))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder()
                ...
                .build();
        var saldoUtregningGrunnlag = SaldoUtregningGrunnlag.forUtregningAvDelerAvUttak(
                List.of(fastsattPeriode), List.of(annenpartsPeriode), grunnlag, utregningsdato);
        var resultat = SaldoUtregningTjeneste.lagUtregning(saldoUtregningGrunnlag);

        assertThat(resultat.saldoITrekkdager(MØDREKVOTE, identifikator)).isEqualTo(Trekkdager.ZERO);
        assertThat(resultat.saldoITrekkdager(MØDREKVOTE, identifikatorNyttArbeidsforhold))
    ...
    void saldoutregning_flerbarnsdager() {
        ...
        var identifikatorNyttArbeidsforhold = forArbeid(new Orgnummer("123"), "789");
        var fastsattPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(fødselsdato, fødselsdato.plusDays(5))
                .periodeResultatType(INNVILGET)
                .flerbarnsdager(true)
                .samtidigUttak(true)
                .aktiviteter(
                        List.of(new FastsattUttakPeriodeAktivitet(new Trekkdager(5), FELLESPERIODE, identifikator)))
                .build();
        var annenpartsPeriode = AnnenpartUttakPeriode.Builder.uttak(fødselsdato, fødselsdato.plusDays(5))
                .innvilget(true)
                .flerbarnsdager(false)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forSelvstendigNæringsdrivende(), MØDREKVOTE, new Trekkdager(5), Utbetalingsgrad.HUNDRED))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder()
        ...
        assertThat(resultat.saldoITrekkdager(MØDREKVOTE, forSelvstendigNæringsdrivende()))
                .isEqualTo(new Trekkdager(14 * 5));
        assertThat(resultat.restSaldoFlerbarnsdager(identifikator)).isEqualTo(new Trekkdager(16 * 5));
        assertThat(resultat.restSaldoFlerbarnsdager(identifikatorNyttArbeidsforhold))
                .isEqualTo(new Trekkdager(16 * 5));
        assertThat(resultat.saldoITrekkdager(FELLESPERIODE, identifikator)).isEqualTo(new Trekkdager(32 * 5));
        assertThat(resultat.saldoITrekkdager(FELLESPERIODE, identifikatorNyttArbeidsforhold))
                .isEqualTo(new Trekkdager(32 * 5));
    }
    ...
    void saldoutregning_flerbarnsdager_begge_fellesperiode() {
        ...
        var fastsattPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(
                        fødselsdato.plusWeeks(10), fødselsdato.plusWeeks(10).plusDays(5))
                .periodeResultatType(INNVILGET)
                .flerbarnsdager(true)
                .aktiviteter(
                        List.of(new FastsattUttakPeriodeAktivitet(new Trekkdager(5), FELLESPERIODE, identifikator)))
        ...
        var annenpartsPeriode = AnnenpartUttakPeriode.Builder.uttak(
                ...
                .flerbarnsdager(true)
                .samtidigUttak(true)
                .uttakPeriodeAktiviteter(List.of(new AnnenpartUttakPeriodeAktivitet(
                        forSelvstendigNæringsdrivende(), FELLESPERIODE, new Trekkdager(5), Utbetalingsgrad.HUNDRED)))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder()
    ...
    void saldoutregning_flerbarnsdager_begge_fellesperiode_berørt() {
        ...
        var annenpartsPeriode = AnnenpartUttakPeriode.Builder.uttak(
                ...
                .uttakPeriodeAktiviteter(List.of(new AnnenpartUttakPeriodeAktivitet(
                        forSelvstendigNæringsdrivende(), FELLESPERIODE, new Trekkdager(5), Utbetalingsgrad.HUNDRED)))
                .build();
        var saldoUtregningGrunnlag = SaldoUtregningGrunnlag.forUtregningAvHeleUttaket(
                List.of(fastsattPeriode), true, List.of(annenpartsPeriode), kontoer.build(), null, null, false);
        var resultat = SaldoUtregningTjeneste.lagUtregning(saldoUtregningGrunnlag);
    ...
    void bfhr_enkel_minsterett_og_litt_msp() {
        ...
        var utregningsdato = LocalDate.MAX;
        var identifikator = forArbeid(new Orgnummer("123"), "456");
        var avslåttPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(fødselsdato.plusWeeks(6), fødselsdato.plusWeeks(16).minusDays(1))
                .periodeResultatType(AVSLÅTT)
                .aktiviteter(List.of(new FastsattUttakPeriodeAktivitet(mspTD, FORELDREPENGER, identifikator)))
                .build();
        var innvilgetPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(
                        fødselsdato.plusWeeks(16), fødselsdato.plusWeeks(19).minusDays(1))
                .periodeResultatType(INNVILGET)
                .resultatÅrsak(ANNET)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(innvilgetMinsterettTD, FORELDREPENGER, identifikator)))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                ...
                .build();
        var saldoUtregningGrunnlag = SaldoUtregningGrunnlag.forUtregningAvDelerAvUttak(
                List.of(avslåttPeriode, innvilgetPeriode), List.of(), grunnlag, utregningsdato);
        var resultat = SaldoUtregningTjeneste.lagUtregning(saldoUtregningGrunnlag);

        assertThat(resultat.saldoITrekkdager(FORELDREPENGER, identifikator))
                .isEqualTo(foreldrepengerTD.subtract(mspTD).subtract(innvilgetMinsterettTD));
        ...
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, false))
                .isEqualTo(foreldrepengerTD
                        .subtract(mspTD)
                        .subtract(innvilgetMinsterettTD)
                        .subtract(minsterettTD)
                        .add(innvilgetMinsterettTD));
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, true))
                .isEqualTo(foreldrepengerTD.subtract(mspTD).subtract(innvilgetMinsterettTD));
    }
    ...
    void bfhr_enkel_minsterett_vs_innvilget_mye_godkjent_aktivitet() {
        ...
        var identifikator = forArbeid(new Orgnummer("123"), "456");
        var avslåttPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(fødselsdato.plusWeeks(6), fødselsdato.plusWeeks(7).minusDays(1))
                .periodeResultatType(AVSLÅTT)
                .aktiviteter(
                        List.of(new FastsattUttakPeriodeAktivitet(new Trekkdager(5), FORELDREPENGER, identifikator)))
                .build();
        var innvilgetPeriode = new FastsattUttakPeriode.Builder()
                .tidsperiode(fødselsdato.plusWeeks(7), fødselsdato.plusWeeks(45).minusDays(1))
                .periodeResultatType(INNVILGET)
                .resultatÅrsak(INNVILGET_FORELDREPENGER_KUN_FAR_HAR_RETT)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(39 * 5), FORELDREPENGER, identifikator)))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder().arbeidsforhold(new Arbeidsforhold(identifikator)))
        ...
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, false))
                .isEqualTo(Trekkdager.ZERO.subtract(minsterettTD));
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, true))
                .isEqualTo(Trekkdager.ZERO);
    }
    ...
    void bfhr_enkel_minsterett_vs_innvilget_med_godkjent_aktivitet_og_minsterett() {
        ...
        var avslåttPeriode = new FastsattUttakPeriode.Builder()
                ...
                .aktiviteter(
                        List.of(new FastsattUttakPeriodeAktivitet(new Trekkdager(5), FORELDREPENGER, identifikator)))
                .build();
        var innvilgetPeriode1 = new FastsattUttakPeriode.Builder()
                .tidsperiode(fødselsdato.plusWeeks(7), fødselsdato.plusWeeks(11).minusDays(1))
                .periodeResultatType(INNVILGET)
                .resultatÅrsak(ANNET)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(4 * 5), FORELDREPENGER, identifikator)))
                .build();
        var innvilgetPeriode2 = new FastsattUttakPeriode.Builder()
                .tidsperiode(
                        fødselsdato.plusWeeks(11), fødselsdato.plusWeeks(45).minusDays(1))
                .periodeResultatType(INNVILGET)
                .resultatÅrsak(INNVILGET_FORELDREPENGER_KUN_FAR_HAR_RETT)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(35 * 5), FORELDREPENGER, identifikator)))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder().arbeidsforhold(new Arbeidsforhold(identifikator)))
                ...
                .build();
        var saldoUtregningGrunnlag = SaldoUtregningGrunnlag.forUtregningAvDelerAvUttak(
                List.of(avslåttPeriode, innvilgetPeriode1, innvilgetPeriode2), List.of(), grunnlag, utregningsdato);
        var resultat = SaldoUtregningTjeneste.lagUtregning(saldoUtregningGrunnlag);

        assertThat(resultat.saldoITrekkdager(FORELDREPENGER, identifikator)).isEqualTo(Trekkdager.ZERO);
        assertThat(resultat.restSaldoMinsterett(identifikator)).isEqualTo(new Trekkdager(40 - 20));
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, false))
                .isEqualTo(new Trekkdager(20 - 40));
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, true))
                .isEqualTo(Trekkdager.ZERO);
    }
    ...
    void bfhr_enkel_minsterett_vs_innvilget_med_mer_uttak_enn_minsterett() {
        ...
        var innvilgetPeriode = new FastsattUttakPeriode.Builder()
                ...
                .resultatÅrsak(ANNET)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(10 * 5), FORELDREPENGER, identifikator)))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder().arbeidsforhold(new Arbeidsforhold(identifikator)))
                ...
                .build();
        var saldoUtregningGrunnlag = SaldoUtregningGrunnlag.forUtregningAvDelerAvUttak(
                List.of(innvilgetPeriode), List.of(), grunnlag, utregningsdato);
        var resultat = SaldoUtregningTjeneste.lagUtregning(saldoUtregningGrunnlag);
        ...
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, false))
                .isEqualTo(new Trekkdager(200 - 50));
        assertThat(resultat.nettoSaldoJustertForMinsterett(FORELDREPENGER, identifikator, true))
                .isEqualTo(new Trekkdager(200 - 50));
    }
    ...
    void skal_arve_saldo_flere_ganger() {
        ...
        var fastsattPeriode1 = new FastsattUttakPeriode.Builder()
                .tidsperiode(LocalDate.of(2019, 12, 18), LocalDate.of(2019, 12, 18))
                .periodeResultatType(INNVILGET)
                .aktiviteter(
                        List.of(new FastsattUttakPeriodeAktivitet(new Trekkdager(50), FELLESPERIODE, identifikator)))
                .build();
        var fastsattPeriode2 = new FastsattUttakPeriode.Builder()
                .tidsperiode(LocalDate.of(2019, 12, 19), LocalDate.of(2019, 12, 19))
                .periodeResultatType(INNVILGET)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(50), FELLESPERIODE, identifikator),
                        new FastsattUttakPeriodeAktivitet(
                                new Trekkdager(50), FELLESPERIODE, identifikatorNyttArbeidsforhold1)))
                .build();
        var fastsattPeriode3 = new FastsattUttakPeriode.Builder()
                .tidsperiode(LocalDate.of(2019, 12, 20), LocalDate.of(2019, 12, 20))
                .periodeResultatType(INNVILGET)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(50), FELLESPERIODE, identifikator),
                        new FastsattUttakPeriodeAktivitet(
                                new Trekkdager(50), FELLESPERIODE, identifikatorNyttArbeidsforhold1),
                        new FastsattUttakPeriodeAktivitet(
                                new Trekkdager(50), FELLESPERIODE, identifikatorNyttArbeidsforhold2)))
                .build();
        var grunnlag = RegelGrunnlagTestBuilder.create()
                .søknad(new Søknad.Builder().type(Søknadstype.FØDSEL))
                .arbeid(new Arbeid.Builder()
        ...
        assertThat(resultat.saldoITrekkdager(FELLESPERIODE, identifikatorNyttArbeidsforhold1))
                .isEqualTo(new Trekkdager(5));
        assertThat(resultat.saldoITrekkdager(FELLESPERIODE, identifikatorNyttArbeidsforhold2))
                .isEqualTo(new Trekkdager(5));
    }
    ...
    void skal_regne_riktig_flerbarnsdager_hvis_annenpart_har_nyoppstartet_arbeidsforhold() {
        ...
        var fastsattPeriode = new FastsattUttakPeriode.Builder()
                ...
                .periodeResultatType(INNVILGET)
                .aktiviteter(List.of(
                        new FastsattUttakPeriodeAktivitet(new Trekkdager(1), FELLESPERIODE, søkersArbeidsforhold)))
                .build();
        var annenpartsArbeidsforhold1 = forArbeid(new Orgnummer("123"), "789");
        var annenpartsArbeidsforhold2 = forSelvstendigNæringsdrivende();
        var annenpartPeriode1 = AnnenpartUttakPeriode.Builder.uttak(
                        LocalDate.of(2019, 12, 17), LocalDate.of(2019, 12, 17))
                .flerbarnsdager(true)
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        annenpartsArbeidsforhold1, FELLESPERIODE, new Trekkdager(1), Utbetalingsgrad.HUNDRED))
                .build();
        var annenpartPeriode2 = AnnenpartUttakPeriode.Builder.uttak(
                        LocalDate.of(2019, 12, 19), LocalDate.of(2019, 12, 19))
                .flerbarnsdager(false)
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        annenpartsArbeidsforhold1, FELLESPERIODE, new Trekkdager(1), Utbetalingsgrad.HUNDRED))
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        annenpartsArbeidsforhold2, FELLESPERIODE, new Trekkdager(1), Utbetalingsgrad.HUNDRED))
                .build();
        var saldoUtregningGrunnlag = SaldoUtregningGrunnlag.forUtregningAvHeleUttaket(
                List.of(fastsattPeriode),
                false,
                List.of(annenpartPeriode1, annenpartPeriode2),
                kontoer.build(),
                null,
                null,
                false);
        ...
        assertThat(resultat.saldoITrekkdager(FELLESPERIODE, søkersArbeidsforhold))
                .isEqualTo(new Trekkdager(97));
        assertThat(resultat.restSaldoFlerbarnsdager(søkersArbeidsforhold)).isEqualTo(new Trekkdager(49));
    }

    @DisplayName(
            "FAGSYSTEM-204667 Oppholdsperiode hos søker ligger før endringsdato, slik at den ikke knekkes mot annen parts perioder")
    @Test
    void oppholdsperiode_hos_søker_som_ikke_er_knekt() {
        var opphold = new FastsattUttakPeriode.Builder()
                ...
                .build();
        var uttakEtterOpphold = new FastsattUttakPeriode.Builder()
                .periodeResultatType(INNVILGET)
                ...
                .build();
        var annenpartUttaksperiode1 = AnnenpartUttakPeriode.Builder.uttak(
                        opphold.getFom(), opphold.getFom().plusWeeks(1).minusDays(1))
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(25), Utbetalingsgrad.FULL))
                .build();
        var annenpartUttaksperiode2 = AnnenpartUttakPeriode.Builder.uttak(
                        annenpartUttaksperiode1.getTom().plusDays(1),
                        opphold.getTom().minusWeeks(1))
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(25), Utbetalingsgrad.FULL))
                .build();
        var annenpartUttaksperiode3 = AnnenpartUttakPeriode.Builder.uttak(
                        annenpartUttaksperiode2.getTom().plusDays(1), opphold.getTom())
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(25), Utbetalingsgrad.FULL))
                .build();
        ...
        var grunnlag = SaldoUtregningGrunnlag.forUtregningAvHeleUttaket(
                List.of(opphold, uttakEtterOpphold),
                false,
                List.of(annenpartUttaksperiode1, annenpartUttaksperiode2, annenpartUttaksperiode3),
                kontoer.build(),
    ...
    void oppholdsperiode_hos_søker_der_annenpart_ikke_har_fylt_hele_perioden() {
        ...
        var annenpartUttaksperiode = AnnenpartUttakPeriode.Builder.uttak(opphold.getFom(), LocalDate.of(2022, 2, 2))
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(3), Utbetalingsgrad.FULL))
                .build();
        var kontoer = new Kontoer.Builder().konto(konto(FELLESPERIODE, 100));
        ...
        var grunnlag = SaldoUtregningGrunnlag.forUtregningAvHeleUttaket(
                List.of(opphold, uttakEtterOpphold),
                false,
                List.of(annenpartUttaksperiode),
                kontoer.build(),
    ...
    void oppholdsperiode_hos_søker_der_annenpart_har_hull_i_oppholdsperioden() {
        ...
        var annenpartUttaksperiode1 = AnnenpartUttakPeriode.Builder.uttak(opphold.getFom(), LocalDate.of(2022, 2, 1))
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(2), Utbetalingsgrad.FULL))
                .build();
        var annenpartUttaksperiode2 = AnnenpartUttakPeriode.Builder.uttak(
                        LocalDate.of(2022, 2, 4), LocalDate.of(2022, 2, 4))
                .innvilget(true)
                .uttakPeriodeAktivitet(new AnnenpartUttakPeriodeAktivitet(
                        forFrilans(), FELLESPERIODE, new Trekkdager(1), Utbetalingsgrad.FULL))
                .build();
        var kontoer = new Kontoer.Builder().konto(konto(FELLESPERIODE, 100));
        ...
        var grunnlag = SaldoUtregningGrunnlag.forUtregningAvHeleUttaket(
                List.of(opphold, uttakEtterOpphold),
                false,
                List.of(annenpartUttaksperiode1, annenpartUttaksperiode2),
                kontoer.build(),
                LocalDateTime.of(annenpartUttaksperiode1.getFom(), LocalTime.NOON),
                LocalDateTime.of(opphold.getFom(), LocalTime.NOON),
                true);
    ...
    void oppholdsperioder_skal_ikke_telle_dager_ved_fritt_uttak() {
        ...
        var grunnlag = SaldoUtregningGrunnlag.forUtregningAvHeleUttaket(
                List.of(opphold, uttakEtterOpphold),
                false,
                List.of(annenpartOpphold, annenpartUttak),
                kontoer,
                null,
                null,
                false);
