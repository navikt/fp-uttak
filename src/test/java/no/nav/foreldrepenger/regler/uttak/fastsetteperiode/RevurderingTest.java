...
class RevurderingTest {
    ...
    void revurderingSøknadUtenSamtykkeOgOverlappendePerioderSkalFørTilManuellPgaSamtykke() {
        ...
        var grunnlag = basicBuilder(oppgittPeriode).rettOgOmsorg(samtykke(false))
            .annenPart(annenPart(lagPeriode(Stønadskontotype.FELLESPERIODE, FAMILIEHENDELSE_DATO.plusWeeks(10), FAMILIEHENDELSE_DATO.plusWeeks(12),
                Utbetalingsgrad.TEN, false).build()))
            .build();
    ...
    void berørtBehandlingHvorDenAndrePartenHarUtbetalingOver0OgSamtidigUttakSkalManueltVurderes() {
        ...
        var grunnlag = basicBuilder(oppgittPeriode).rettOgOmsorg(samtykke(true))
            .annenPart(annenPart(lagPeriode(Stønadskontotype.FELLESPERIODE, FAMILIEHENDELSE_DATO.plusWeeks(10), FAMILIEHENDELSE_DATO.plusWeeks(12),
                Utbetalingsgrad.HUNDRED.subtract(Utbetalingsgrad.TEN), true).build()))
            .behandling(berørtBehandling())
            .build();
    ...
    void revurdering_søknad_der_opphørsdato_ligger_i_perioden() {
        var uttaksperiode = uttakPeriode(Stønadskontotype.FELLESPERIODE, FAMILIEHENDELSE_DATO.plusWeeks(10), FAMILIEHENDELSE_DATO.plusWeeks(12));

        var grunnlag = basicBuilder(uttaksperiode).medlemskap(new Medlemskap.Builder().opphørsdato(uttaksperiode.getFom().plusWeeks(1))).build();
    ...
    void revurdering_søknad_der_opphørsdato_ligger_før_perioden() {
        ...
        var grunnlag = basicBuilder(uttaksperiode).medlemskap(new Medlemskap.Builder().opphørsdato(uttaksperiode.getFom().minusWeeks(1))).build();
    ...
    private AnnenpartUttakPeriode.Builder lagPeriode(Stønadskontotype stønadskontotype,
        ...
        return AnnenpartUttakPeriode.Builder.uttak(fom, tom)
            ...
            .uttakPeriodeAktivitet(
                new AnnenpartUttakPeriodeAktivitet(AktivitetIdentifikator.forArbeid(new Orgnummer("000000003"), null), stønadskontotype,
