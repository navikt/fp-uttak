...
class GraderingOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    protected final FastsettePerioderRegelOrkestrering fastsettePerioderRegelOrkestrering =
            new FastsettePerioderRegelOrkestrering();
    ...
    void når_graderingsperiode_går_tom_skal_perioden_dras_til_søkers_fordel() {
        var kontoer =
                new Kontoer.Builder().flerbarnsdager(17 * 5).konto(konto(FORELDREPENGER, 57 * 5));
        var søknadsperiode1 =
                oppgittPeriode(FORELDREPENGER, LocalDate.of(2019, 1, 23), LocalDate.of(2019, 3, 1));
        // Søker vil gå tom for dager i løpet av 19. sept, derfor får søker en ekstra trekkdager
        // (Søkers fordel)
        var søknadsperiode2 =
                gradertoppgittPeriode(
                        FORELDREPENGER,
        ...
        var grunnlag =
                RegelGrunnlagTestBuilder.create()
                        ...
                        .rettOgOmsorg(
                                new RettOgOmsorg.Builder().morHarRett(false).aleneomsorg(true))
                        .søknad(
                                new Søknad.Builder()
                                        .type(Søknadstype.FØDSEL)
                                        .oppgittPeriode(søknadsperiode1)
                                        .oppgittPeriode(søknadsperiode2))
                        .build();
    ...
    @Test
    void
            samtidig_uttaksprosent_skal_settes_til_100_minus_gradering_arbeidstidsprosent_hvis_samtidig_uttak_og_gradering() {
        var fødselsdato = LocalDate.of(2018, 1, 1);
        var grunnlag = RegelGrunnlagTestBuilder.create();
        leggPåKvoter(grunnlag);
        var arbeidstidsprosent = BigDecimal.TEN;
        var samtidigUttaksprosent = new SamtidigUttaksprosent(50);
        var gradertMedSamtidigUttak =
        ...
        assertThat(resultat.get(2).uttakPeriode().getSamtidigUttaksprosent())
                .isEqualTo(SamtidigUttaksprosent.HUNDRED.subtract(arbeidstidsprosent));
    }

    @Test
    void
            uttak_på_to_arbeidsforhold_hvor_den_ene_går_tom_for_fellesperiode_skal_føre_0_utbetaling_og_0_trekkdager_for_den_som_går_tom() {
        var fødselsdato = LocalDate.of(2018, 1, 1);
                        ...
                        new Søknad.Builder()
                                ...
                                .oppgittPeriode(
                                        gradertoppgittPeriode(
                                                ...
                                                fødselsdato.plusWeeks(33).minusDays(1),
                                                prosent50,
                                                Set.of(ARBEIDSFORHOLD_1))))
                .kontoer(kontoer)
                .arbeid(
        ...
        assertThat(resultat.get(4).uttakPeriode().getUtbetalingsgrad(ARBEIDSFORHOLD_2))
    ...
    @Test
    void
            uttak_på_to_arbeidsforhold_hvor_den_ene_går_tom_for_mødrekvote_skal_føre_0_utbetaling_og_0_trekkdager_for_den_som_går_tom() {
        var fødselsdato = LocalDate.of(2018, 1, 1);
        var prosent50 = new BigDecimal("50.00");
        var grunnlag = RegelGrunnlagTestBuilder.create();
        var kontoer =
        ...
        var fastsettePeriodeGrunnlag = grunnlag.build();
        var resultat =
                fastsettePerioderRegelOrkestrering.fastsettePerioder(fastsettePeriodeGrunnlag);
        ...
        assertKontoOgResultat(resultat.get(2), MØDREKVOTE, INNVILGET);
        assertTrekkdager(
                resultat.get(2), ARBEIDSFORHOLD_1, new Trekkdager(new BigDecimal("22.50")));
        assertTrekkdager(resultat.get(2), ARBEIDSFORHOLD_2, new Trekkdager(9 * 5));
        assertKontoOgResultat(resultat.get(3), MØDREKVOTE, INNVILGET);
        assertThat(resultat.get(3).uttakPeriode().getGraderingIkkeInnvilgetÅrsak()).isNull();
        assertTrekkdager(
                resultat.get(3), ARBEIDSFORHOLD_1, new Trekkdager(new BigDecimal("22.50")));
        ...
        assertTrekkdager(
                resultat.get(4), ARBEIDSFORHOLD_1, new Trekkdager(new BigDecimal("22.50")));
        assertThat(resultat.get(4).uttakPeriode().getUtbetalingsgrad(ARBEIDSFORHOLD_2))
                .isEqualTo(Utbetalingsgrad.ZERO);
        assertTrekkdager(resultat.get(4), ARBEIDSFORHOLD_2, new Trekkdager(45));
    }

    private void assertKontoOgResultat(
            FastsettePeriodeResultat fastsettePeriodeResultat,
            Stønadskontotype stønadskontotype,
            Perioderesultattype perioderesultattype) {
        assertThat(fastsettePeriodeResultat.uttakPeriode().getStønadskontotype())
                .isEqualTo(stønadskontotype);
        assertThat(fastsettePeriodeResultat.uttakPeriode().getPerioderesultattype())
                .isEqualTo(perioderesultattype);
    }

    private void assertTrekkdager(
            FastsettePeriodeResultat fastsettePeriodeResultat,
            AktivitetIdentifikator aktivitetIdentifikator,
            Trekkdager trekkdager) {
        assertThat(fastsettePeriodeResultat.uttakPeriode().getTrekkdager(aktivitetIdentifikator))
                .isEqualTo(trekkdager);
    }

    @Test
    void
            utbetalingsgrad_og_trekkdager_skal_ta_utgangspunkt_samtidig_uttaksprosent_for_aktiviteter_uten_gradering_hvis_det_finnes_gradering() {
        var fødselsdato = LocalDate.of(2018, 1, 1);
        var grunnlag = RegelGrunnlagTestBuilder.create();
        var arbeidstidsprosent = BigDecimal.TEN;
        var samtidigUttaksprosent = new SamtidigUttaksprosent(50);
        ...
        assertThat(resultat.get(2).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_2))
                .isEqualTo(new Trekkdager(9));
        assertThat(
                        resultat.get(2)
                                .uttakPeriode()
                                .getUtbetalingsgrad(ARBEIDSFORHOLD_2)
                                .decimalValue())
                .isEqualTo(
                        BigDecimal.valueOf(100)
                                .subtract(arbeidstidsprosent)
    ...
    void periode_med_gradering_og_90_prosent_arbeid_og_lite_igjen_på_saldo_skal_bli_innvilget() {
        ...
        var kontoer =
                new Kontoer.Builder()
                        .konto(konto(FELLESPERIODE, 5)) // bare 5 igjen
                        .konto(konto(FORELDREPENGER_FØR_FØDSEL, 1000))
                        .konto(konto(MØDREKVOTE, 1000));
        var grunnlag =
                basicGrunnlag(fødselsdato)
                        .søknad(
                                new Søknad.Builder()
    ...
    @Test
    void
            periode_med_gradering_og_80_prosent_arbeid_og_det_er_for_lite_igjen_på_saldo_slik_at_perioden_knekkes() {
        var fødselsdato = LocalDate.of(2018, 1, 1);
        var graderingFom = fødselsdato.plusWeeks(10);
        var graderingTom = fødselsdato.plusWeeks(20).minusDays(1);
        var grunnlag = RegelGrunnlagTestBuilder.create();
        var kontoer =
                new Kontoer.Builder()
                        .konto(konto(FELLESPERIODE, 5)) // bare 5 dager felles igjen
                        .konto(konto(FORELDREPENGER_FØR_FØDSEL, 15))
                        .konto(konto(MØDREKVOTE, 50));
        grunnlag.datoer(new Datoer.Builder().fødsel(fødselsdato))
                ...
                .søknad(
                        new Søknad.Builder()
                                ...
                                .oppgittPeriode(
                                        oppgittPeriode(
                                                MØDREKVOTE, fødselsdato, graderingFom.minusDays(1)))
                                .oppgittPeriode(
                                        gradertoppgittPeriode(
                                                FELLESPERIODE,
                                                graderingFom,
    ...
    void skal_knekke_riktig_ved_flere_graderingsperioder_og_flere_arbeidsforhold() {
        ...
        var fastsattAktiviter =
                List.of(
                        new FastsattUttakPeriodeAktivitet(
                                new Trekkdager(96), FORELDREPENGER, ARBEIDSFORHOLD_1),
                        new FastsattUttakPeriodeAktivitet(
                                new Trekkdager(96), FORELDREPENGER, ARBEIDSFORHOLD_2),
                        new FastsattUttakPeriodeAktivitet(
                                Trekkdager.ZERO, FORELDREPENGER, ARBEIDSFORHOLD_3));
        var fastsattPeriode =
                new FastsattUttakPeriode.Builder()
                        .periodeResultatType(INNVILGET)
                        .tidsperiode(
                                omsorgsovertakelse.minusDays(1), omsorgsovertakelse.minusDays(1))
                        ...
                        .søknad(
                                new Søknad.Builder()
                                        ...
                                        .oppgittPeriode(
                                                gradertoppgittPeriode(
                                                        FORELDREPENGER,
                                                        omsorgsovertakelse.plusWeeks(1).plusDays(1),
                                                        omsorgsovertakelse.plusWeeks(10),
                                                        BigDecimal.valueOf(75),
                                                        Set.of(
                                                                ARBEIDSFORHOLD_1,
                                                                ARBEIDSFORHOLD_2))))
        ...
        assertThat(resultat.get(2).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_1))
                .isEqualTo(new Trekkdager(1.5));
        assertThat(resultat.get(2).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_2))
                .isEqualTo(new Trekkdager(1.5));
        assertThat(resultat.get(2).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_3))
                .isEqualTo(new Trekkdager(6));
    ...
    void skal_knekke_riktig_når_arbeidsforhold_som_ikke_er_gradert_går_tom_for_dager() {
        ...
        assertThat(resultat.get(0).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_1))
                .isEqualTo(new Trekkdager(2));
        assertThat(resultat.get(0).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_2))
                .isEqualTo(new Trekkdager(4));
        ...
        assertThat(resultat.get(1).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_1))
                .isEqualTo(new Trekkdager(new BigDecimal("0.5")));
        assertThat(resultat.get(1).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD_2))
    ...
    void opphevet_gradering_skal_ikke_endre_på_hvilket_arbeidsforhold_det_er_søkt_gradering_for() {
        ...
        var resultat = fastsettPerioder(grunnlag);
        assertThat(resultat).hasSize(3);
        assertThat(resultat.get(1).uttakPeriode().getGraderingIkkeInnvilgetÅrsak())
                .isEqualTo(GraderingIkkeInnvilgetÅrsak.AVSLAG_PGA_FOR_TIDLIG_GRADERING);
        ...
        assertThat(resultat.get(1).uttakPeriode().erGraderingInnvilget(ARBEIDSFORHOLD_1)).isFalse();
        assertThat(
                        resultat.get(1).uttakPeriode().getAktiviteter().stream()
                                .anyMatch(
                                        a ->
                                                a.isSøktGradering()
                                                        && a.getIdentifikator()
                                                                .equals(ARBEIDSFORHOLD_1)))
                .isTrue();
        assertThat(
                        resultat.get(1).uttakPeriode().getAktiviteter().stream()
                                .anyMatch(
