...
class ManglendeSøktOrkestreringTest extends FastsettePerioderRegelOrkestreringTestBase {
    ...
    void skal_avslå_og_trekke_foreldrepenger_for_bare_far_har_rett_hvis_dager_igjen() {
        ...
        var grunnlag = basicGrunnlagFar(fødselsdato)
                .søknad(søknad(
                        FØDSEL, oppgittPeriode(FORELDREPENGER, fødselsdato.plusWeeks(50), fødselsdato.plusWeeks(52))))
        ...
        var fastsattPeriode = new FastsattUttakPeriode.Builder()
                ...
                .aktiviteter(List.of(
                        ...
                        new FastsattUttakPeriodeAktivitet(
                                new Trekkdager(0), FORELDREPENGER, forSelvstendigNæringsdrivende())))
    ...
    void manglende_søkt_periode_før_nytt_arbeidsforhold_tilkommer() {
        ...
        assertThat(perioder.get(0).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD))
                .isEqualTo(new Trekkdager(Virkedager.beregnAntallVirkedager(
                        fødselsdato.plusWeeks(6), startdatoNyttArbeidsforhold.minusDays(1))));
        ...
        assertThat(perioder.get(2)
                        .uttakPeriode()
                        .getTrekkdager(tilkommetArbeidsforhold)
                        .merEnn0())
                .isTrue();
    }
    ...
    void skal_avslå_foreldrepenger_når_msp_innenfor_første_6_ukene_bare_mor_har_rett() {
        ...
        assertThat(perioder.get(3).uttakPeriode().getUtbetalingsgrad(ARBEIDSFORHOLD))
                .isEqualTo(Utbetalingsgrad.HUNDRED);
        assertThat(perioder.get(3).uttakPeriode().getTrekkdager(ARBEIDSFORHOLD).merEnn0())
                .isTrue();
    ...
    void bfhr_msp_skal_ikke_avslås_pga_tom_på_konto_hvis_dager_igjen_på_minsteretten() {
        ...
        var fpFørMsp = OppgittPeriode.forVanligPeriode(
                FORELDREPENGER,
                ...
                MORS_AKTIVITET_GODKJENT);
        var fpEtterMsp = OppgittPeriode.forVanligPeriode(
                FORELDREPENGER,
    ...
    void msp_pga_fødselshendelse_etter_termin_skal_innvilge_mødrekvote() {
        ...
        var aktivitet =
                grunnlag.getArbeid().getAktiviteter().stream().findFirst().orElseThrow();
    ...
    void msp_første_6_ukene_som_ikke_er_opprettet_av_fødselshendelse_skal_avlås() {
        ...
        var mødrekvote2 = oppgittPeriode(
                MØDREKVOTE, fødselsdato.plusWeeks(4), fødselsdato.plusWeeks(5).minusDays(1));
        // hull mellom 5-6
        var mødrekvote3 = oppgittPeriode(
                MØDREKVOTE, fødselsdato.plusWeeks(6), fødselsdato.plusWeeks(7).minusDays(1));
        var grunnlag = basicGrunnlagMor(fødselsdato)
                .datoer(new Datoer.Builder().fødsel(fødselsdato).termin(termindato))
