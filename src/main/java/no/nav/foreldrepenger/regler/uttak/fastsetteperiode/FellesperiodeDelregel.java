...
import no.nav.foreldrepenger.regler.uttak.fastsetteperiode.betingelser.SjekkOmPeriodenStarterFørLovligUttakFørFamiliehendelse;
...
public class FellesperiodeDelregel implements RuleService<FastsettePeriodeGrunnlag> {
    ...
    private Specification<FastsettePeriodeGrunnlag> sjekkOmMorGjelderFødsel() {
        return rs.hvisRegel(SjekkOmSøknadGjelderFødsel.ID, SjekkOmSøknadGjelderFødsel.BESKRIVELSE)
            .hvis(new SjekkOmSøknadGjelderFødsel(), sjekkOmPeriodenStarterFørLovligUttakFørFamiliehendelse())
    ...
    private Specification<FastsettePeriodeGrunnlag> sjekkOmPeriodenStarterFørLovligUttakFørFamiliehendelse() {
        return rs.hvisRegel(SjekkOmPeriodenStarterFørLovligUttakFørFamiliehendelse.ID, SjekkOmPeriodenStarterFørLovligUttakFørFamiliehendelse.BESKRIVELSE)
                .hvis(new SjekkOmPeriodenStarterFørLovligUttakFørFamiliehendelse(konfigurasjon), IkkeOppfylt.opprett("UT1040", IkkeOppfyltÅrsak.MOR_SØKER_FELLESPERIODE_FØR_12_UKER_FØR_TERMIN_FØDSEL, false, false))
    ...
    private Specification<FastsettePeriodeGrunnlag> sjekkSaldoForMor() {
        ...
        Specification<FastsettePeriodeGrunnlag> noenTilgjengligeDagerNode =
            rs.hvisRegel(SjekkOmTilgjengeligeDagerPåNoenAktiviteteneForSøktStønadskonto.ID, "Er det tilgjengelige dager på fellesperioden?")
                ...
                .ellers(Manuellbehandling.opprett("UT1043", IkkeOppfyltÅrsak.IKKE_STØNADSDAGER_IGJEN, Manuellbehandlingårsak.STØNADSKONTO_TOM, true, false));
    ...
    private Specification<FastsettePeriodeGrunnlag> delFlytForTidligUttak() {
        ...
        var noenDisponibleDagerNode =
            rs.hvisRegel(SjekkOmTilgjengeligeDagerPåNoenAktiviteteneForSøktStønadskonto.ID, SjekkOmTilgjengeligeDagerPåNoenAktiviteteneForSøktStønadskonto.BESKRIVELSE)
                ...
                .ellers(Manuellbehandling.opprett("UT1257", IkkeOppfyltÅrsak.IKKE_STØNADSDAGER_IGJEN, Manuellbehandlingårsak.STØNADSKONTO_TOM, true, false));
