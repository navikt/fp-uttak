...
import static no.nav.foreldrepenger.regler.uttak.fastsetteperiode.Virkedager.justerHelgTilMandag;
...
public class TomKontoIdentifiserer {
    ...
    public static Optional<TomKontoKnekkpunkt> identifiser(
        ...
        for (var aktivitet : aktiviteter) {
            ...
            if (saldoUtregning.getMaxDagerUtenAktivitetskrav().merEnn0()) {
                finnDatoDagerUtenAktivitetskravOppbrukt(
                                uttakPeriode,
                                aktivitet,
                                saldoUtregning,
                                skalTrekkeDager,
            ...
            if (farRundtFødselIntervall != null
                    && FarUttakRundtFødsel.erKontoRelevant(stønadskontotype)
                    && saldoUtregning.getFarUttakRundtFødselDager().merEnn0()) {
                // Fra lovendring 2022: Far kan ta ut 10 dager samtidig rundt fødsel (før termin,
                // etter fødsel) + far alene kan ta ut fritt før uke 6.
    ...
    private static Optional<LocalDate> finnDatoKontoGårTomIPeriode(
    ...
    private static Optional<LocalDate> finnDatoKontoGårTomIPeriodeFlerbarnsdager(
            OppgittPeriode oppgittPeriode,
            AktivitetIdentifikator aktivitet,
            SaldoUtregning saldoUtregning,
            boolean skalTrekkeDager) {
    ...
    private static Optional<LocalDate> finnDatoMinsterettOppbrukt(
            OppgittPeriode oppgittPeriode,
            AktivitetIdentifikator aktivitet,
            SaldoUtregning saldoUtregning,
            boolean skalTrekkeDager,
    ...
    private static Optional<LocalDate> finnDatoNesteStønadsperiodeOppbrukt(
            OppgittPeriode oppgittPeriode,
            AktivitetIdentifikator aktivitet,
            SaldoUtregning saldoUtregning,
            boolean skalTrekkeDager,
    ...
    private static Optional<LocalDate> finnDatoDagerUtenAktivitetskravOppbrukt(
            OppgittPeriode oppgittPeriode,
            AktivitetIdentifikator aktivitet,
            SaldoUtregning saldoUtregning,
            boolean skalTrekkeDager,
    ...
    private static Optional<LocalDate> datoHvisSaldoOppbruktIPeriode(
            OppgittPeriode oppgittPeriode, AktivitetIdentifikator aktivitet, Trekkdager saldo) {
    ...
    private static int saldoTilVirkedagerSamtidigUttak(OppgittPeriode periode, Trekkdager saldo) {
        return saldoTilVirkedagerVedRedusertUttak(
                saldo, periode.getSamtidigUttaksprosent().decimalValue());
    }
