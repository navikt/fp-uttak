...
public class FastsettePeriodeRegel implements RuleService<FastsettePeriodeGrunnlag> {
    ...
    private Specification<FastsettePeriodeGrunnlag> sjekkOmAnnenPartsPeriodeHarUtbetalingsgrad() {
        return rs.hvisRegel(
                        SjekkOmAnnenPartsPeriodeHarUtbetalingsgrad.ID,
    ...
    private Specification<FastsettePeriodeGrunnlag> sjekkPeriodeInnenforMaksgrense() {
        return rs.hvisRegel(
                        SjekkOmPeriodenErEtterMaksgrenseForUttak.ID,
    ...
    private Specification<FastsettePeriodeGrunnlag> sjekkOmPeriodeEtterNesteStønadsperiode() {
        return rs.hvisRegel(
                        SjekkOmPeriodenErEtterNesteStønadsperiode.ID,
                        SjekkOmPeriodenErEtterNesteStønadsperiode.BESKRIVELSE)
                .hvis(
                        new SjekkOmPeriodenErEtterNesteStønadsperiode(),
    ...
    private Specification<FastsettePeriodeGrunnlag> kreverBehandlingenSammenhengendeUttak() {
        return rs.hvisRegel(
                ...
                .hvis(
                        ...
                        new UtsettelseDelregelSammenhengendeUttak().getSpecification())
    ...
    private Specification<FastsettePeriodeGrunnlag> sjekkOmSøktGradering100ProsentEllerMer() {
        return rs.hvisRegel(
                        SjekkOmSøktGraderingHundreProsentEllerMer.ID,
                        "Er søkt arbeid 100 prosent eller mer i perioden?")
                .hvis(
                        new SjekkOmSøktGraderingHundreProsentEllerMer(),
                        Manuellbehandling.opprett(
                                "UT1180",
                                IkkeOppfyltÅrsak.ARBEID_HUNDRE_PROSENT_ELLER_MER,
                                Manuellbehandlingårsak.AVKLAR_ARBEID,
                                true,
                                false))
                .ellers(sjekkOmPeriodeErStebarnsadopsjon());
    }
