...
public class FarUttakRundtFødsel {
    ...
    public static Optional<LukketPeriode> utledFarsPeriodeRundtFødsel(
        ...
        return utledFarsPeriodeRundtFødsel(
                false, søknadstype.gjelderTerminFødsel(), datoer.getFamiliehendelse(), datoer.getTermin());
    }
    ...
    public static Optional<LukketPeriode> utledFarsPeriodeRundtFødsel(
        ...
        var farFørTermin = Konfigurasjon.STANDARD
                .getParameterHvisAktivVed(Parametertype.FAR_UTTAK_FØR_TERMIN_UKER, familieHendelseDato)
                .map(Period::ofWeeks)
                .orElse(Period.ZERO);
        var farEtterFødsel = Konfigurasjon.STANDARD
                .getParameterHvisAktivVed(Parametertype.FAR_UTTAK_ETTER_FØDSEL_UKER, familieHendelseDato)
                .map(Period::ofWeeks)
                .orElse(Period.ZERO);
