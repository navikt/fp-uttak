...
public class FarUttakRundtFødsel {
    ...
    public static boolean erPeriodeRelevant(
            LukketPeriode farRundtFødselIntervall, LukketPeriode periode) {
    ...
    public static Optional<LukketPeriode> utledFarsPeriodeRundtFødsel(RegelGrunnlag grunnlag) {
        return utledFarsPeriodeRundtFødsel(
                grunnlag.getDatoer(), grunnlag.getKontoer(), grunnlag.getSøknad().getType());
    }

    public static Optional<LukketPeriode> utledFarsPeriodeRundtFødsel(
            Datoer datoer, Kontoer kontoer, Søknadstype søknadstype) {
        if (!kontoer.harSpesialkonto(Spesialkontotype.FAR_RUNDT_FØDSEL)
                || kontoer.getSpesialkontoTrekkdager(Spesialkontotype.FAR_RUNDT_FØDSEL) == 0
    ...
    public static Optional<LukketPeriode> utledFarsPeriodeRundtFødsel(
        ...
        var farFørTermin =
                Konfigurasjon.STANDARD
                        .getParameterHvisAktivVed(
                                Parametertype.FAR_UTTAK_FØR_TERMIN_UKER, familieHendelseDato)
                        .map(Period::ofWeeks)
                        .orElse(Period.ZERO);
        var farEtterFødsel =
                Konfigurasjon.STANDARD
                        .getParameterHvisAktivVed(
                                Parametertype.FAR_UTTAK_ETTER_FØDSEL_UKER, familieHendelseDato)
                        .map(Period::ofWeeks)
                        .orElse(Period.ZERO);
