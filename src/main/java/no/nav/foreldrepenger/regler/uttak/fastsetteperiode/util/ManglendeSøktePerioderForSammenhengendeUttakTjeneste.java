...
public final class ManglendeSøktePerioderForSammenhengendeUttakTjeneste {
    ...
    private static List<OppgittPeriode> trimPerioder(
            RegelGrunnlag grunnlag, List<OppgittPeriode> manglendeSøktePerioder) {
    ...
    private static boolean contains(List<OppgittPeriode> list, LocalDate fom, LocalDate tom) {
        return list.stream()
                .anyMatch(item -> item.getFom().isEqual(fom) && item.getTom().isEqual(tom));
    }
    ...
    private static Optional<LocalDate> førsteUttaksdatoAnnenpart(RegelGrunnlag grunnlag) {
        if (grunnlag.getAnnenPart() == null
                || grunnlag.getAnnenPart().getUttaksperioder().isEmpty()) {
            return Optional.empty();
        }
    ...
    private static LocalDate førsteFom(List<? extends LukketPeriode> perioder) {
        return perioder.stream()
                .min(Comparator.comparing(LukketPeriode::getFom))
                .orElseThrow()
                .getFom();
    }

    private static Optional<OppgittPeriode> utledManglendeSøktForFar(
    ...
    private static Optional<OppgittPeriode> fjernPerioderFørSkjæringstidspunktOpptjening(
            OppgittPeriode msp, RegelGrunnlag grunnlag) {
        var skjæringstidspunkt = grunnlag.getOpptjening().getSkjæringstidspunkt();
        // Skal ikke fjerne periode før skjæringstidspunkt for far med aleneomsorg eller enerett
    ...
    private static boolean isFarMedAleneomsorg(RegelGrunnlag grunnlag) {
        return grunnlag.getBehandling().isSøkerFarMedMor()
                && grunnlag.getRettOgOmsorg().getAleneomsorg();
    }
    ...
    private static Stream<OppgittPeriode> finnManglendeMellomliggendePerioder(
            RegelGrunnlag grunnlag, List<OppgittPeriode> ekskludertePerioder) {
    ...
    private static List<OppgittPeriode> finnManglendeSøktFørFødsel(
    ...
    private static List<OppgittPeriode> finnPerioderEtterFødsel(
            List<LukketPeriode> søktePerioder,
            LocalDate familiehendelseDato,
            Set<Stønadskontotype> gyldigeStønadskontotyper) {
