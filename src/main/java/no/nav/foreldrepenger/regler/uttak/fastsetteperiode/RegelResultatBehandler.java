...
class RegelResultatBehandler {
    ...
    RegelResultatBehandlerResultat innvilgAktuellPeriode(
        ...
        var innvilget = new UttakPeriode(
                innvilgPeriode,
    ...
    RegelResultatBehandlerResultat avslåAktuellPeriode(
        ...
        var avslåPeriode = knekkpunktOpt
                .map(TomKontoKnekkpunkt::dato)
        ...
        var avslått = new UttakPeriode(
                ...
                regelresultat.getGraderingIkkeInnvilgetÅrsak(),
                lagAktiviteter(
                        avslåPeriode, regelresultat, overlapperInnvilgetAnnenpartsPeriode, SamtidigUttaksprosent.ZERO),
    ...
    private UttakPeriodeAktivitet lagAktivitet(
        ...
        var periodeAktivitetResultat = finnPeriodeAktivitetResultat(
                oppgittPeriode,
                overlapperMedInnvilgetPeriodeHosAnnenpart,
                identifikator,
                regelresultat,
    ...
    private PeriodeAktivitetResultat finnPeriodeAktivitetResultat(
        ...
        var harIgjenTrekkdager =
                isHarIgjenTrekkdager(oppgittPeriode, aktivitet, regelresultat, stønadskonto.orElse(null));
        ...
        if (regelresultat.skalUtbetale()) {
            var utbetalingsgradUtregning =
                    bestemUtbetalingsgradUtregning(oppgittPeriode, aktivitet, annenpartSamtidigUttaksprosent);
        ...
        if (regelresultat.trekkDagerFraSaldo()) {
            if (manuellBehandling && stønadskonto.isEmpty()) {
                ...
                var graderingInnvilget = regelresultat.getGraderingIkkeInnvilgetÅrsak() == null
    ...
    private boolean isHarIgjenTrekkdager(
        ...
        var nettosaldo = saldoUtregning.nettoSaldoJustertForMinsterett(
    ...
    private UtbetalingsgradUtregning bestemUtbetalingsgradUtregning(
        ...
        if (samtidigUttaksprosent != null) {
            return new UtbetalingsgradSamtidigUttakUtregning(
                    samtidigUttaksprosent, oppgittPeriode.getArbeidsprosent(), annenpartSamtidigUttaksprosent);
        }
