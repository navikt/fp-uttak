...
class RegelResultatBehandler {
    ...
    RegelResultatBehandlerResultat innvilgAktuellPeriode(
                        ...
                        .map(
                                k ->
                                        oppgittPeriode.kopiMedNyPeriode(
                                                oppgittPeriode.getFom(), k.minusDays(1)))
        ...
        var innvilget =
                new UttakPeriode(
                        ...
                        regnSamtidigUttaksprosentMotGradering(
                                innvilgPeriode, annenpartSamtidigUttaksprosent),
        ...
        if (knekkpunktOpt.isEmpty()) {
            ...
            var etterKnekk =
                    oppgittPeriode.kopiMedNyPeriode(
                            knekkpunktOpt.get().dato(), oppgittPeriode.getTom());
            return RegelResultatBehandlerResultat.medKnekk(innvilget, etterKnekk);
        }
    }

    RegelResultatBehandlerResultat avslåAktuellPeriode(
            OppgittPeriode oppgittPeriode,
            FastsettePerioderRegelresultat regelresultat,
            Optional<TomKontoKnekkpunkt> knekkpunktOpt,
        ...
        var avslåPeriode =
                knekkpunktOpt
                        ...
                        .map(
                                knekkdato ->
                                        oppgittPeriode.kopiMedNyPeriode(
                                                oppgittPeriode.getFom(), knekkdato.minusDays(1)))
                        .orElse(oppgittPeriode);
        ...
        var avslått =
                new UttakPeriode(
                        ...
                        lagAktiviteter(
                                avslåPeriode,
                                regelresultat,
                                overlapperInnvilgetAnnenpartsPeriode,
                                SamtidigUttaksprosent.ZERO),
                        regnSamtidigUttaksprosentMotGradering(
                                avslåPeriode, SamtidigUttaksprosent.ZERO),
    ...
    private static SamtidigUttaksprosent regnSamtidigUttaksprosentMotGradering(
            OppgittPeriode oppgittPeriode, SamtidigUttaksprosent annenpartSamtidigUttaksprosent) {
    ...
    private Optional<Stønadskontotype> konto(OppgittPeriode oppgittPeriode) {
        return oppgittPeriode.getStønadskontotype() != null
                ? Optional.of(oppgittPeriode.getStønadskontotype())
                : utledKonto(oppgittPeriode);
    }

    RegelResultatBehandlerResultat manuellBehandling(
            OppgittPeriode oppgittPeriode, FastsettePerioderRegelresultat regelresultat) {
        ...
        var resultat =
                new UttakPeriode(
                        ...
                        regelresultat.getGraderingIkkeInnvilgetÅrsak(),
                        lagAktiviteter(
                                oppgittPeriode, regelresultat, false, SamtidigUttaksprosent.ZERO),
                        regnSamtidigUttaksprosentMotGradering(
                                oppgittPeriode, SamtidigUttaksprosent.ZERO),
    ...
    private Set<UttakPeriodeAktivitet> lagAktiviteter(
            OppgittPeriode oppgittPeriode,
            FastsettePerioderRegelresultat regelresultat,
                ...
                .map(
                        a ->
                                lagAktivitet(
                                        a,
                                        regelresultat,
                                        overlapperMedInnvilgetPeriodeHosAnnenpart,
                                        oppgittPeriode,
                                        annenpartSamtidigUttaksprosent))
                .collect(Collectors.toSet());
    }

    private UttakPeriodeAktivitet lagAktivitet(
        ...
        var periodeAktivitetResultat =
                finnPeriodeAktivitetResultat(
                        oppgittPeriode,
                        overlapperMedInnvilgetPeriodeHosAnnenpart,
                        identifikator,
                        regelresultat,
                        annenpartSamtidigUttaksprosent);
        return new UttakPeriodeAktivitet(
                identifikator,
                periodeAktivitetResultat.utbetalingsgrad(),
                periodeAktivitetResultat.trekkdager(),
                søktGradering);
    }

    private PeriodeAktivitetResultat finnPeriodeAktivitetResultat(
            OppgittPeriode oppgittPeriode,
            boolean overlapperMedInnvilgetPeriodeHosAnnenpart,
            AktivitetIdentifikator aktivitet,
            FastsettePerioderRegelresultat regelresultat,
            SamtidigUttaksprosent annenpartSamtidigUttaksprosent) {
        // Må sjekke saldo her, ved flere arbeidsforhold kan det reglene ha gått til sluttpunkt som
        // trekkes dager selv om ett av arbeidsforholdene er tom
        // På arbeidsforholdet som er tom på konto skal det settes 0 trekkdager
        var stønadskonto = konto(oppgittPeriode);
        var harIgjenTrekkdager =
                isHarIgjenTrekkdager(
                        oppgittPeriode, aktivitet, regelresultat, stønadskonto.orElse(null));

        var manuellBehandling = manuellBehandling(regelresultat);
        if (overlapperMedInnvilgetPeriodeHosAnnenpart
                || (!manuellBehandling && !harIgjenTrekkdager)) {
        ...
        if (regelresultat.skalUtbetale()) {
            var utbetalingsgradUtregning =
                    bestemUtbetalingsgradUtregning(
                            oppgittPeriode, aktivitet, annenpartSamtidigUttaksprosent);
        ...
        if (regelresultat.trekkDagerFraSaldo()) {
            if (manuellBehandling && stønadskonto.isEmpty()) {
                ...
                var graderingInnvilget =
                        regelresultat.getGraderingIkkeInnvilgetÅrsak() == null
                                && oppgittPeriode.erSøktGradering(aktivitet);
                trekkdager =
                        TrekkdagerUtregningUtil.trekkdagerFor(
                                oppgittPeriode,
                                graderingInnvilget,
                                oppgittPeriode.getArbeidsprosent(),
                                regnSamtidigUttaksprosentMotGradering(
                                        oppgittPeriode, annenpartSamtidigUttaksprosent));
            }
        }
    ...
    private boolean isHarIgjenTrekkdager(
        ...
        var nettosaldo =
                saldoUtregning.nettoSaldoJustertForMinsterett(
                        stønadskonto, aktivitet, oppgittPeriode.kanTrekkeAvMinsterett());
        if (regelresultat.getAvklaringÅrsak() != null
                && regelresultat.getAvklaringÅrsak().trekkerMinsterett()) {
            ...
            return (nettosaldo.merEnn0()
                    && (minsterettSaldo.merEnn0() || utenAktivitetskravSaldo.merEnn0()));
        }
        return nettosaldo.merEnn0();
    }
    ...
    private void validerKnekkpunkt(OppgittPeriode uttakPeriode, TomKontoKnekkpunkt knekkpunkt) {
        if (!uttakPeriode.overlapper(knekkpunkt.dato())) {
            throw new IllegalArgumentException(
                    "Knekkpunkt må være i periode. " + knekkpunkt.dato() + " - " + uttakPeriode);
        }
    }

    private UtbetalingsgradUtregning bestemUtbetalingsgradUtregning(
            OppgittPeriode oppgittPeriode,
            AktivitetIdentifikator aktivitet,
            SamtidigUttaksprosent annenpartSamtidigUttaksprosent) {
        ...
        var samtidigUttaksprosent =
                regnSamtidigUttaksprosentMotGradering(
                        oppgittPeriode, annenpartSamtidigUttaksprosent);
        if (samtidigUttaksprosent != null) {
            return new UtbetalingsgradSamtidigUttakUtregning(
    ...
    private record PeriodeAktivitetResultat(
            Utbetalingsgrad utbetalingsgrad, Trekkdager trekkdager) {}
}
